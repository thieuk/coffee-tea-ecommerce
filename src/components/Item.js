...
export default function Item(props) {
    ...
    return (
        <div className="flex flex-col items-center bg-white border-2 border-neutral-200 shadow-lg rounded h-fit sm:p-5 py-5 md:w-[240px] sm:w-[180px] w-[150px] md:h-[330px] hover:shadow-2xl hover:scale-[1.01]">
            ...
            <h1 className="w-[80%] text-center sm:text-lg text-sm font-extrabold text-ellipsis overflow-hidden whitespace-nowrap" title={props.brand}>
                {`${props.brand}`}
            </h1>
            <h2 className="w-[80%] text-center sm:text-base text-xs text-ellipsis font-bold overflow-hidden whitespace-nowrap" title={props.item}>{`${props.item}`}</h2>
            {props.sale ? 
                <p>

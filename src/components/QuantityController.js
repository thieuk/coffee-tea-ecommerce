...
export default function QuantityController(prop) {
    ...
    return (
        <div className="flex">
            <div className="w-fit border-2 border-blue-600 rounded mt-2">
                ...
                <input type="text" value={cart.map(item => { if (item.id === prop.product.id) return item.quantity; })} disabled className="text-center w-7"/>
            ...
            { prop.removeButton ? <button onClick={() => handleRemoveItem(prop.product.id)} className="h-[30px] ml-5 mt-2 px-2 rounded text-white bg-red-600 text-sm">remove</button> : <></> }
        </div>
    );
}